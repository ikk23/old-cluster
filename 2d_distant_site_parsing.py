...
"""
Use this file for parsing 2d_distant_site_spatial.slim file output.

Each individual .sh file will call this python script.

python 2d_distant_site_parsing.py 2d_distant_site_spatial.slim <interest_drive>
<changing> <param1=param1value> <param2=param2value> <init>

interest_drive for this includes: tads_aut, tade_supp, tads_y, and wt_drop
...
changing includes:
    ...
    4. inbreeding: then param1 is "avoidance=" and param2 is "penalty="
...
def generate_slim(filename,
    ...
    defineConstant(\"TADS_Y_SUPPRESSION\", {});\n\
    defineConstant(\"TARE\", {});\n\
    defineConstant(\"TARE_UNDERDOMINANCE\", {});\n\
    ...
    defineConstant(\"INBREEDING_FECUNDITY_PENALTY\", {});\n    /*".format(
                    ...
                    "T" if tads_y_suppression else "F",
                    "T" if tare else "F",
                    "T" if tare_underdominance else "F",
...
def cfg_params():
    ...
    """
    # Default values:
    # may need to manually change!
    capacity = 50000
    density_interaction_distance = 0.01
    fitness = 0.8
    drop_size = 505
    drop_radius = 0.01
    embryo_resistance_rate = 0.76
    germline_resistance_rate = 0.8
    beta = 12
    ...
    tads_y_suppression = False
    tare = False
    tare_underdominance = False
    track_by_cell = True
    ...
    for arg in sys.argv:
        ...
        if arg.startswith("tads_aut"):
            tads_autosomal_suppression = True
        if arg.startswith("tads_y"):
            tads_y_suppression = True
        ...
        if arg.startswith("efficiency="):
            ...
            embryo_resistance_rate = 0.95*germline_resistance_rate #95% efficiency
    ...
    heterozygous_drop, male_only_drop, num_grnas, r1_rate, speed, tade,\
    tade_suppression, tade_double, tads_autosomal_suppression, tads_modification,\
    tads_y_suppression, tare, tare_underdominance, track_by_cell, track_ripleys_l,\
    number_of_cells, no_drop, x_linked_drive, inbreeding_avoidance_factor, inbreeding_fecundity_penalty,\
    print_csv_header)
...
def main():
    ...
    for arg in sys.argv:
        if arg.startswith("beta_speed"):
            ...
            interest_param2 = ("Migration Rate", params[12])
        ...
        if arg.startswith("all"):
            interest_param1 = ("Fitness", params[2])
            interest_param2 = ("Efficiency", params[6])
            interest_param3 = ("Beta", params[7])
            interest_param4 = ("Speed", params[12])
        if arg.startswith("resistance"):
            interest_param1 = ("R1 Rate", params[11])
